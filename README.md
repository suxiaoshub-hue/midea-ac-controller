...
```text
C:\Users\你的用户名\.midea_ac_controller\wks_hosts.json
```
...
```json
{
  "_interval": 3,
  "_timeout": 800,
  "双人包（59-60）": ["192.168.1.59", "192.168.1.60"],
  "三人包（67-69）": ["192.168.1.67", "192.168.1.68", "192.168.1.69"],
  "单人包（70）": ["192.168.1.70"]
}
```

`_interval` 是 ping 间隔秒数，`_timeout` 是单次 ping 超时时间毫秒。卡片名称要和软件里的空调名称一致。
